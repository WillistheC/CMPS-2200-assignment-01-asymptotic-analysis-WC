...
3. **Parallelism and recursion**
  ...
  - 3d (4 pts): Recursive calls are made on $2$ lists of size $\frac{n}{2}$, so $W(n) = 2W(\frac{n}{2}) = Θ(n)$, because the recursive calls split the list in half, $S(n) = S(\frac{n}{2}) = Θ(\mathrm{log}n)$

  - 3e (5 pts): The work does not change from parallelizing, and only the longest branch counts towards the span, which still leads to $Θ(\mathrm{log}n)$ because the branches are equal
